...
initialize(){
    source "${VDT_ROOT:="/opt/nesi/vdt"}/utils/common.sh"
}
...
$VDT_ROOT"
	...
	# # CUDA
	# export VDT_CPATH="/usr/local/cuda/include:$CPATH"
	# export VDT_PATH="/usr/local/cuda/bin:$PATH"
	# export VDT_LD_LIBRARY_PATH="/usr/local/cuda/lib64:$LD_LIBRARY_PATH"
	# export VDT_CUDA_HOME="/usr/local/cuda"


    # Set paths
	export VDT_PATH="$VDT_PATH/opt/slurm/sbin"
	export VDT_CPATH="$VDT_CPATH:/opt/slurm/include"
	export VDT_LD_LIBRARY_PATH="$VDT_LD_LIBRARY_PATH:/opt/slurm/lib64:/opt/slurm/lib64/slurm"
	export VDT_LIBRARY_PATH="$VDT_LIBRARY_PATH:/opt/slurm/lib64:/opt/slurm/lib64/slurm"
    ...
    for ev in $(compgen -A variable | grep ^VDT );do
        export "SINGULARITYENV_$ev"="${!SINGULARITYENV_$ev}:${!ev}"
        debug "SINGULARITYENV_$ev"="${!SINGULARITYENV_$ev}:${!ev}"
    done
